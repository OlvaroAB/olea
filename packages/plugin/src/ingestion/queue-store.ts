...
export class ObsidianQueueStore implements QueueStore {
   ...
   * since this store last loaded, and this must not clobber it. Atomic
   * (`readModifyWrite`) when `this.host` supports it — see `../retrieval/
   * serializing-data-host.ts`'s module doc for why serializing individual
   * `loadData`/`saveData` calls is not, on its own, enough — falling back
