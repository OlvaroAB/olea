 ...
 *
 * F8.4a/`[D-176]` (`ol-r1by`) added a third hand-off here, but it moved out
 * (`ol-2zfj.55`): `createObsidianAcceptNoteOfferPort` never actually touched
 * `App` — only `VaultSource` — and this file's own opening sentence is
 * exactly the reason that matters: importing anything from this file pulls
 * in `./view.js`'s real, VALUE-level `import { ItemView } from 'obsidian'`,
 * and that npm package ships no runtime code at all (confirmed directly —
 * `Failed to resolve entry for package "obsidian"` under this package's
 * Vitest config), so nothing in this file can ever be unit-tested, port or
 * not. `createObsidianAcceptNoteOfferPort` now lives in `./ports.ts`
 * (alongside `createVaultPruneInstrumentPort`, its `App`-free sibling) and
 * is merely re-exported below so `main.ts`'s existing `from
 * './registry/obsidian-ports.js'` import keeps working unedited — see
 * `./ports.ts`'s own doc for the full move rationale and
 * `createObsidianAcceptNoteOfferPort`'s doc there for what it does.
 */
...
export { createObsidianAcceptNoteOfferPort } from './ports.js';
