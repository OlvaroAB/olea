 ...
 *
 * `extract.ts` extracts tiers 1 and 2 (her concept notes, her `topic`
 * properties — curated, authored identity). This module is the third leg:
 * concepts **derived from her material** — specifically the two feeds F4.1
 * names as new for this bead, both landed as dependencies (P5-T01's
 ...
 * invisible to this pass — passes a richer `vocabulary` (`extract.ts` does
 * exactly this for its own tier-3 minting). The concept's **display name**
 * stays whatever the vocabulary said, never the derived text's own casing
 * (R2: speak in hers).
 *
...
export interface ExtractTier3EvidenceOptions extends RegisterSourcesOptions {
   ...
   * "identity without inventing it." `extract.ts` passes a richer
 ...
 * because it is already covered upstream by `./extract.js`'s tier-1/2 pass
 * over her `topic` properties. So this row genuinely contributes nothing HERE,
 * and says so, rather than showing a bare zero that reads like a failed
 * extraction.
 */
...
/**
 * A note's courses, read exactly the way `extract.ts` reads a lecture note's
 * (`./course.js`, F1.3, `ol-jbnu`): its own `course` property when it has
 * one — a list, so M:N course association holds for generated-content
 * citations exactly as it does for tier-2 ones — and otherwise the course
 * folder it lives under. One derivation, both call sites: two subtly
 * different answers to "which course is this?" is how a concept and its own
 * evidence end up disagreeing.
 */
